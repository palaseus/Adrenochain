...
// Test additional bridge manager functions for better coverage
func TestBridgeManagerAdditionalFunctions(t *testing.T) {
	bm := NewBridgeManager()
	...
	// Test AddValidator
	validator, _ := NewValidator("val1", "0x1234567890abcdef", "eth", big.NewInt(1000000))
	err := bm.AddValidator(validator)
	if err != nil {
	...
	// Test adding validator to non-existent chain
	invalidValidator, _ := NewValidator("val2", "0xfedcba0987654321", "non_existent", big.NewInt(1000000))
	err = bm.AddValidator(invalidValidator)
	if err == nil {
		t.Error("Expected error for non-existent chain")
	}
	...
	err = bm.FailTransfer("tx1", "Test failure")
	if err != nil {
		t.Errorf("FailTransfer failed: %v", err)
	}

	// Test failing already completed transfer
	err = bm.FailTransfer("tx1", "Test failure")
	if err == nil {
		t.Error("Expected error for already failed transfer")
	}

	// Test GetBridge
	retrievedBridge, err := bm.GetBridge("eth_bsc")
	if err != nil {
		t.Errorf("GetBridge failed: %v", err)
	}
	if retrievedBridge == nil {
		t.Error("GetBridge should return bridge")
	}

	// Test GetBridge with non-existent ID
	_, err = bm.GetBridge("non_existent")
	if err == nil {
		t.Error("Expected error for non-existent bridge")
	}

	// Test GetBridgesByChain
	bridges := bm.GetBridgesByChain("eth")
	if len(bridges) == 0 {
		t.Error("GetBridgesByChain should return bridges for existing chain")
	}

	// Test GetBridgesByChain with non-existent chain
	bridges = bm.GetBridgesByChain("non_existent")
	if bridges != nil {
		t.Error("GetBridgesByChain should return nil for non-existent chain")
	}

	// Test UpdateBridgeStatus
	err = bm.UpdateBridgeStatus("eth_bsc", BridgeInactive)
	if err != nil {
		t.Errorf("UpdateBridgeStatus failed: %v", err)
	}

	// Test UpdateBridgeStatus with non-existent bridge
	err = bm.UpdateBridgeStatus("non_existent", BridgeActive)
	if err == nil {
		t.Error("Expected error for non-existent bridge")
	}

	// Test AddValidatorToBridge
	err = bm.AddValidatorToBridge("eth_bsc", "val1")
	if err != nil {
		t.Errorf("AddValidatorToBridge failed: %v", err)
	}

	// Test RemoveValidatorFromBridge
	err = bm.RemoveValidatorFromBridge("eth_bsc", "val1")
	if err != nil {
		t.Errorf("RemoveValidatorFromBridge failed: %v", err)
	}

	// Test GetTotalTransfers
	totalTransfers := bm.GetTotalTransfers()
	if totalTransfers != 1 {
		t.Errorf("Expected 1 transfer, got %d", totalTransfers)
	}
}
