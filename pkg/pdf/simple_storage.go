...
func NewSimplePDFStorage(baseDir string) (*SimplePDFStorage, error) {
	...
	if err := validatePath(baseDir); err != nil {
		return nil, fmt.Errorf("invalid base directory path: %w", err)
	}
...
func validatePath(path string) error {
	...
	// Allow absolute paths for test environments (like /tmp)
	if filepath.IsAbs(path) && !strings.HasPrefix(path, "/tmp") {
		return fmt.Errorf("absolute paths not allowed (except /tmp for testing)")
	}
	...
	for _, char := range suspiciousChars {
		if strings.Contains(path, char) {
			return fmt.Errorf("path contains suspicious character: %s", char)
		}
	}
